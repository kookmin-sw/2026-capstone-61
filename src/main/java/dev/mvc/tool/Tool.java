...
public class Tool {
  ...
  public static synchronized String getOSPath() {
      String configured = System.getProperty("dog.upload.base");
      if (configured == null || configured.isBlank()) {
        configured = System.getenv("DOG_UPLOAD_BASE");
      }
      ...
      if (configured != null && !configured.isBlank()) {
        ...
        base = "C:/kd/deploy/paw";
      } else {
        base = "/home/ubuntu/deploy/paw";
      }

      return Paths.get(base).toString().replace("\\", "/");
  }

  public static synchronized String getUploadPath(String... parts) {
    Path path = Paths.get(getOSPath(), parts);
